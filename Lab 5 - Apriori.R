...
library (arulesViz)
#inbuild dataset
data("Groceries"); #data=>to read inbuild dataset
...
#Confidence to find which is stronger
...
#Adjustments to find better confidence levels
grocery_rules <- apriori(Groceries, parameter = list(support = 0.02, confidence = 0.20))
grocery_rules
a<-inspect(head(sort(grocery_rules, by = "confidence"), 2));
a
